...
void dante_verify::update_miner(const string &enclave_public_key, const Address &reward_address, const string &enclave_signature) {
...
bool dante_verify::require_auth(const string &message, const string &enclave_signature) {
	return true;
}

// Test signature
void dante_verify::test(const string &message, const string &enclave_signature) {
	DEBUG(message);
	...
	auto ret = platon::platon_ecrecover(h256(hashed_value, sizeof(hashed_value)), fromHex(enclave_signature), recovered_address);
	DEBUG(ret);
	DEBUG(recovered_address.toString());
}

// Submit enclave proof
void dante_verify::submit_proof(const string &enclave_public_key, const string &enclave_timestamp, const u128 &enclave_capacity, const string &enclave_signature) {
	require_auth(enclave_public_key, enclave_signature);
...
proof_of_capacity dante_verify::get_submit_proof(const string &enclave_public_key) {
...
u128 dante_verify::get_total_capacity() {
	return total_capacity.self();
}

// Submit miner info
void dante_verify::submit_miner_info(const string &name, const string &peer_id, const string &country_code, const string &url) {
