...
(async function () {
  ...
  try {
    let balance = await blockchain.contractCall(tokenContract,'balanceOf', [marketContractAddress]);
    console.log('marketContractAddress balance: ' + balance + ' DAT');

    balance = await blockchain.contractCall(tokenContract,'balanceOf', [verifyContractAddress]);
    console.log('verifyContractAddress balance : ' + balance + ' DAT');

    balance = await blockchain.contractCall(tokenContract,'balanceOf', ['lat120swfan2f50myx2g5kux4t8la9ypsz94dhh5ex']);
    console.log( 'lat120swfan2f50myx2g5kux4t8la9ypsz94dhh5ex balance: ' + balance + ' DAT');
